  ...
  <div class="projects">
    ...
    <div class="content-wrapper" v-for="(item,key) in items" :key="key">
      ...
      <p class="description">
        ...
        {{item.future}}由于本项目编写时间较早，彼时对于js的认知稍浅，现在回看可改进空间比较大，将来会在简化逻辑、丰富业务、压缩代码上下功夫；
      </p>
      <p class="danger">{{item.caution}}注意：本项目还在拓展维护，所有依赖资源储存在github，浏览时加载可能较慢；可使用移动端高版本浏览器。</p>
    </div>
    <div>
      <!-- 日记 -->
      <div class="content-wrapper">
        ...
        <div class="gallery">
          <img v-for="(item,key) in gallery" :key :src="{gallery.diary}" alt class="ImgCell">
        </div>
        <p class="description">
      ...
      items: [
        {
          ...
          devtool:'JS Stylus;',
          character: '1、用JS实现indexedDB数据库操作，相比Cookies和localStorage，数据结构化，运用后端数据库逻辑；2、完整的日记的写入、输出、分类、筛选、修改、删除、添加图片、添加多图功能；3、响应式布局，对pc和移动端分别优化（虽然没下一个项目的解决方案完美）；4、自制modal框体、toast提示浮层、各个组件动画（滑动、淡入、淡出等）、改造美化了Input单选框；5、首页背景css轮播；6、日记展示区的图片可依据数量不同而显示不同布局、单张图片时将采用大图框的显示布局、同时根据每张图片长宽比实现最好的展示方式；7、canvas气泡、水母背景（模仿自codepen的一个项目），以及自制了一个算数小游戏；'
        },
